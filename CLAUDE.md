...
- `DowTheory_Trend_V1.0.pine` - Dow Theory Trend indicator (Pine Script v6)
  ...
  - Entry: Dow Theory Confirmation (strict-adjacency 3-swing) — LL → HH → HL confirm → Buy (ห้ามมี LH คั่น), HH → LL → LH confirm → Sell (ห้ามมี HL คั่น); track ผ่าน `prevSwingType` + `lastSwingType` shift 2 slot ทุกครั้งที่มี swing ใหม่, ทุก swing ที่แทรกจะยกเลิก pattern อัตโนมัติ; pattern range สำหรับ 1R ใช้ HH-LL; SL วางที่ ent±oneR เสมอ (ไม่มี SL Edge Mode แล้ว)
  ...
  - Entry Mode: **Pullback อย่างเดียว** — สร้าง pending order ที่ระดับ pullback (`% of Price` หรือ `% of Range` วางเลย HL/LH ไปอีก X%) รอราคา dip/bounce แตะจึง fill, มี expiry เป็นจำนวนแท่ง, ยกเลิกทันทีถ้าสัญญาณตรงข้ามยิงตามมา หรือมี swing ใหม่แทรก (ปิดไปแล้ว: Market / Breakout / Dow Breakout); ถ้า pullback level ถูกราคาผ่านไปแล้ว → ทิ้ง signal (แสดง label `"✗ Pullback level passed"` — ไม่มี market fallback)
