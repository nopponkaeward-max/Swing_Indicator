...
- `MegaphoneV1.0.pine` - Megaphone Breakout Trading System (Pine Script v6, **ตรวจจับ + วาด + เทรดเบรคเอาท์จากกล่อง**)
  ...
  - State machine: pattern ใหม่ก่อตัว → วาดเส้นขอบ + กล่องครอบ + label `◀ BULL/BEAR MEGAPHONE ▶`; ยังถ่างต่อ → ยืดขอบตาม (**การขยายกรอบไม่ใช่ breakout**); หยุดถ่าง (เจอ LH/HL) → freeze: ถ้า close ยังอยู่ในกรอบ → **arm OCO plan** ที่ขอบกรอบสุดท้าย พร้อมยืดขอบขวาของกล่องตามราคาไปจนกว่าจะเบรค/หมดอายุ/มี pattern ใหม่; ถ้า close หลุดกรอบไปแล้ว (pivot ยืนยันช้า) → `useStopFill` (default ON) ถือว่า stop order โดน trigger แล้ว **fill ที่ขอบกรอบ** (tag "Buy/Sell Stop", วงกลมอยู่ที่ขอบพอดี), ปิด toggle → label `MISSED BREAK`
  - Breakout = **OCO Buy Stop / Sell Stop**: วาง pending Buy Stop ที่ขอบบน (planTop) + Sell Stop ที่ขอบล่าง (planBot) พร้อมกัน; แท่ง confirmed แรกที่ high > planTop → BUY fill ที่ planTop / low < planBot → SELL fill ที่ planBot (บวก/ลบ spread; gap ข้าม → fill ที่ open, tag `· Gap`); ฝั่งใดโดน trigger → ยกเลิกอีกฝั่ง (`hasPlan:=false` ลบทั้งสองเส้น); เบรคสองฝั่งแท่งเดียว → ทิศตามสีแท่ง; ห้ามเบรคแท่งเดียวกับที่ arm; plan หมดอายุตาม `inpPlanExpiry` แท่ง (กันออเดอร์ผีไกลจากกล่อง); pattern ใหม่ confirm → ยกเลิก plan + ปิดทุก position ที่ close; **day filter เช็คตอนเข้าเทรด** (ไม่ใช่ตอน arm) — เบรควันที่ปิดเทรด → label `SKIP (Day Filter)` + ยกเลิก plan; entry ทุกแบบผ่าน `f_openTrade()` (5 labels/entry — TradingView จำกัด 500 labels ทั้ง chart, วงกลม trade เก่ามากๆ จะถูกลบก่อนกล่อง)
