...
- `DowTheory_Trend_V1.0.pine` - Dow Theory Trend indicator (Pine Script v6)
  ...
  - Entry Mode สไตล์ V10.1: (1) Market — เข้าที่ close ของแท่ง confirm ทันที; (2) Pullback — สร้าง pending order ที่ระดับ pullback (`% of Price` หรือ `% of Range` ของ HH↔LL) รอราคาถอยแตะจึง fill, มี expiry เป็นจำนวนแท่ง, ยกเลิก pending ทันทีถ้าสัญญาณตรงข้ามยิงตามมา; (3) Breakout — สร้าง pending order ที่ระดับ HH (ขาขึ้น) หรือ LL (ขาลง) รอราคาเบรคผ่านจึง fill, ยกเลิกทันทีถ้ามี swing ใหม่ (HH/HL/LH/LL) ปรากฏก่อนเบรค, ใช้ expiry เดียวกับ Pullback; (4) Dow Breakout — เหมือน Breakout แต่เพิ่ม filter: ต้อง "ราคาไม่เคยเบรค HH หลัง HH pivot ก่อน HL confirm" (mirror LL/LH สำหรับ SELL) ไม่งั้นทิ้ง pattern (แสดง label `"✗ Dow filter"`); SL เลือกได้ระหว่าง "At Pivot" (HL/LH) หรือ "% of HH-HL Range from HH" (เช่น 100 = HL ตรง, 120 = ต่ำกว่า HL 20% ของ range = buffer); Reverse Trade ไม่มีผลในโหมดนี้ (บังคับ LONG/SHORT ตาม pattern)
  ...
  - **1R = Actual SL Distance toggles** (แก้ปัญหา RR ที่ตั้งไม่ตรงกับระยะจริง): `inpSlEdgeSelfR` (สำหรับ SL Edge Mode ปกติ modes 1-3) และ `inpDowSelfR` (สำหรับ Dow Breakout mode 4) — เมื่อเปิด: `effOneR = |entry - SL|` → TP distance = SL distance × RR ตรงเป๊ะ, lot ปรับให้ risk = Risk per 1R ($) พอดี, R accounting ตรงกับ RR ที่ตั้ง; ปิด (default) = พฤติกรรมเดิม (TP/lot ใช้ 1R จาก 1R Basis, RR อาจเบี้ยว); ไม่กระทบเมื่อ SL Edge Mode = OFF (SL อยู่ที่ ent±oneR อยู่แล้ว)
